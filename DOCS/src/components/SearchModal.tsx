...
const SearchModal = ({ isOpen, onClose }: SearchModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-4 md:pt-[15vh] px-4 bg-slate-900/40 backdrop-blur-sm" onClick={onClose}>
      <div 
      ...
      >
        <div className="flex items-center px-4 border-b border-slate-100 py-4">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none text-slate-900 text-lg placeholder-slate-400 min-w-0"
            value={query}
          ...
          {results.length > 0 ? (
            <div className="space-y-1">
              {results.map((item, idx) => (
                <div
                  key={item.path}
                  className={`flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-lg cursor-pointer transition-all ${
                    idx === selectedIndex ? 'bg-primary-50 border-primary-100' : 'hover:bg-slate-50 border-transparent'
                  } border`}
                  onClick={() => handleSelect(item)}
                  onMouseEnter={() => setSelectedIndex(idx)}
                >
                  <div className={`p-2 rounded-lg ${idx === selectedIndex ? 'bg-white text-primary-600 shadow-sm' : 'bg-slate-100 text-slate-400'} hidden sm:block`}>
                    <FileText className="w-5 h-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`font-bold text-sm ${idx === selectedIndex ? 'text-primary-700' : 'text-slate-900'}`}>{item.title}</p>
                      <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider bg-slate-100 px-1.5 py-0.5 rounded whitespace-nowrap">
                        {item.category}
                      </span>
                    </div>
        ...
        <div className="px-4 py-3 bg-slate-50 border-t border-slate-100 hidden sm:flex items-center justify-between text-[10px] text-slate-400 font-medium">
